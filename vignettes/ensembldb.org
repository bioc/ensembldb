...
=select= method. First we fetch all transcripts for the genes /ZBTB16/, /BCL2/ and
...
only the protein coding transcripts for these genes. Note that the results in
the first query are ordered according to the specified keys while in the second
query the ordering is arbitrary. Ordering by keys is only performed if =keys= is a
character vector or a single filter object; for multiple filters the data is
returned in an arbitrary ordering.
...
+ The ordering of the results returned by the =genes=, =exons=, =transcripts=
  ...
  filter objects. The only exception is the =select= method where the ordering of
  the results matches the ordering of the =keys= or of the values of the filter is
  a single filter is submitted.
